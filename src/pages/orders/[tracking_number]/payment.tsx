...
import NotificationButton from '@/components/orders/notification-button';
...
const OrderView = ({ order, loadingStatus }: OrderViewProps) => {
  ...
  let taxes = localStorage.getItem('tax');
  let taxAmount = (order?.amount! * taxes!) / 100;
  useEffect(() => {
    resetCart();
  }, []);
  ...
  const { price: discount } = usePrice({ amount: order?.discount! });
            ...
            <div className="mt-12 flex flex-col md:flex-row">
              <div className="mb-10 w-full md:mb-0 md:w-1/2 ltr:md:pr-3 rtl:md:pl-3">
                ...
                <div>
                  ...
                  {order?.discount > 0 ? (
                    <Listitem title={t('text-discount')} details={discount} />
                  ) : null}
                  ...
                  {order?.note ? (
                    <Listitem title="Note" details={order?.note} />
                  ) : null}
                </div>
              </div>
              {/* end of total amount */}
